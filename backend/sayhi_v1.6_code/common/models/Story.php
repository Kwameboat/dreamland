...
class Story extends \yii\db\ActiveRecord
{
    ...
    public function getLastTweleveMonthStory()
    {
        ...
        $res = \common\helpers\MonthlyGraphQuery::query(
            "SELECT month(from_unixtime(created_at)) as month, count(id) as total_ad FROM story where status=10 and from_unixtime(created_at) >= DATE_SUB(CURDATE(), INTERVAL 1 YEAR) group by month",
            "SELECT EXTRACT(MONTH FROM to_timestamp(created_at))::int AS month, count(id) AS total_ad FROM story WHERE status=10 AND to_timestamp(created_at) >= (CURRENT_DATE - INTERVAL '1 year') GROUP BY month"
        );
